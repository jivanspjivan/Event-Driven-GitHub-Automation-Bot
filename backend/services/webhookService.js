...
const executeRule = async (rule, repository, payload) => {
  if (rule.action_type === 'record_event') {
    return [{ actionType: 'record_event', status: 'completed' }];
  }
    ...
    return [
      {
        ...
        status: 'failed',
        errorMessage: `Unsupported action: ${rule.action_type}`,
      },
    ];
  }

  const issueNumber = Number(payload.issue?.number);
  const githubResult = await captureAction('github_issue_triage', async () => {
  ...
  const slackResult = await captureAction('slack_notification', () =>
  ...
  return [githubResult, slackResult];
};
...
const processDelivery = async ({ deliveryId, eventName, payload }) => {
  ...
  try {
    ...
    for (const rule of applicableRules) {
      actionResults.push(...(await executeRule(rule, repository, payload)));
    }
    ...
    const status = failedAction ? 'failed' : executedActionCount > 0 ? 'completed' : 'ignored';
    const errorMessage = failedAction?.errorMessage || null;
    await pool.query(
      `UPDATE webhook_deliveries
          SET status = $2, matched_rule_count = $3, executed_action_count = $4,
              error_message = $5, action_results = $6, processed_at = NOW()
        WHERE id = $1`,
      [
...
const listDeliveries = async (userId, limit = 50) => {
  const result = await pool.query(
    `SELECT webhook_deliveries.delivery_id, webhook_deliveries.event_name,
            ...
            webhook_deliveries.error_message, webhook_deliveries.action_results,
...
module.exports = { captureAction, isRuleApplicable, processDelivery, listDeliveries };
